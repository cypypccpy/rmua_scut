...
class BackBootAreaBehavior {
 public:
  ...
  bool LoadParam(const std::string &proto_file_path) {
    ...
    boot_position_.pose.position.x = decision_config.blue_master_bot().start_position().x();
    boot_position_.pose.position.z = decision_config.blue_master_bot().start_position().z();
    boot_position_.pose.position.y = decision_config.blue_master_bot().start_position().y();

    auto master_quaternion = tf::createQuaternionMsgFromRollPitchYaw(decision_config.blue_master_bot().start_position().roll(),
